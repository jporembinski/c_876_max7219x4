 ...
 *
 * Created on 11 stycznia 2017, 23:55
 *
 * Ostateczna wersja z dnia 27 kwietnia 2017 r.
 *
 */
...
void main(void)
{
    ...
    OPTION_REG = (OPTION_REG & 0b01111111); // Wyzeruj bit RBPU (włącz podciąganie na porcie B)
    ADCON1 = 0x06;
    TRISB = 0b11111111;
    TRISC = 0b00011111;
    LOAD = 1;
    ...
    while(1) {
        ...
        if(sekundy & 0x01) {
        for(k=0;k<8;k++)
            Display(k+1, cyfra1[dg*8+k]<<1, k+1, cyfra1[8*jg+k]<<2, k+1, cyfra2[8*dm+k], k+1, cyfra1[8*jm+k]<<1);
        }
        else {
        for(k=0;k<8;k++)    
            Display(k+1, cyfra1[dg*8+k]<<1, k+1, cyfra1[8*jg+k]<<2, k+1, cyfra1[8*dm+k], k+1, cyfra1[8*jm+k]<<1);
        }
    }
    return;
}
